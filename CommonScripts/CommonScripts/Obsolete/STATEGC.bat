...
echo ^>Entering STATEGC.bat>>%_LOGFILE%
echo ^>Entering STATEGC.bat
...
for /F "skip=1" %%i in (%_SUBTASKSLISTFILE%) do (
    ...
    set _THIS_TESTREPORTFILE=!_THIS_CMPOUTFOLDER!\log\index.html
    ...
    powershell -Command "(gc !_THIS_ERRORS!) -replace 'bgColor=#FF0000', 'bgColor=\"#FF0000\"' -replace 'bgColor=red', 'bgColor=\"red\"' -replace '&', '*and*' -replace 'bgColor=#D0D0D0', 'bgColor=\"#D0D0D0\"' -replace '.html', '.ontop' -replace 'HREF=\"', 'HREF=\"!_THIS_CMPOUTFOLDER!\diff\' | Out-File -encoding ASCII !_THIS_ERRORS!"

    REM Append errors found as a HTML fragment
    rem type %_STAT_HTMLTABHEADER% >>%_STAT_COLLECTEDERRORSFILE%
    rem echo ^<tr^>^<td colspan="4"^>!_THIS_TASKNAME!^</td^>^</tr^> >>%_STAT_COLLECTEDERRORSFILE%
    rem type !_THIS_ERRORS! >>%_STAT_COLLECTEDERRORSFILE%
    rem type %_STAT_HTMLTABFOOTER% >>%_STAT_COLLECTEDERRORSFILE%
